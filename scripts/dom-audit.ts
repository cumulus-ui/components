...
// ─── Internal Class Prefix Normalization ──────────────────────

/**
 * Read known prefixes from internal style files.
 * Each file like `abstract-switch.ts` produces prefix `abstract-switch`,
 * meaning classes like `.abstract-switch--wrapper` → `.wrapper`.
 */
function getInternalPrefixes(): Set<string> {
  const stylesDir = resolve(ROOT, 'src/internal/styles');
  if (!fs.existsSync(stylesDir)) return new Set();
  return new Set(
...
/** Strip known internal prefix from a class name. */
function normalizeClass(cls: string, prefixes: Set<string>): string {
  const idx = cls.indexOf('--');
  if (idx > 0) {
    const prefix = cls.slice(0, idx);
    if (prefixes.has(prefix)) {
      return cls.slice(idx + 2);
    }
  }
  return cls;
}

/** Normalize all class names in a DOM tree, deduplicating after strip. */
function normalizeDOM(node: DOMNode, prefixes: Set<string>): DOMNode {
  return {
    ...node,
    classes: [...new Set(node.classes.map(c => normalizeClass(c, prefixes)))].sort(),
    children: node.children.map(child => normalizeDOM(child, prefixes)),
  };
}
  ...
  try {
    if (isCapture) {
      ...
      for (const comp of components) {
        ...
        const rawOurDOM = await extractOurDOM(ourPage, ourUrl);
        const ourDOM = rawOurDOM ? normalizeDOM(rawOurDOM, prefixes) : null;
    ...
    for (const s of servers) {
      try { if (s.pid) process.kill(-s.pid); } catch {}
    }
  }
}
