...
.cartesian-chart--vertical-marker {
  stroke: var(--color-charts-line-axis, #dedee3);
  stroke-width: 1px;
}
...
.cartesian-chart--axis-label {
  font-weight: bold;
  fill: var(--color-text-body-default, #0f141a);
}

.cartesian-chart--grid {
  stroke: var(--color-charts-line-grid, #dedee3);
  stroke-width: 1px;
}

.cartesian-chart--ticks__line {
  stroke: var(--color-charts-line-tick, #dedee3);
  stroke-width: 1px;
}

.cartesian-chart--ticks__text {
  font-size: var(--font-chart-detail-size, 12px);
  fill: var(--color-text-body-secondary, #424650);
}
...
.cartesian-chart--vertical-marker-circle {
  fill: var(--color-background-container-content, #ffffff);
  pointer-events: none;
  stroke-width: var(--border-line-chart-width, 2px);
}
...
.cartesian-chart--chart-container-outer.cartesian-chart--fit-height:not(.cartesian-chart--axis-label + .cartesian-chart--chart-container-outer.cartesian-chart--fit-height, .cartesian-chart--chart-container-outer.cartesian-chart--fit-height.cartesian-chart--has-filters) {
  margin-block-start: calc(0.5 * var(--font-chart-detail-size, 12px));
}
