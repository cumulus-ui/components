...
export interface TableProps<T = any> {
   ...
   * * `header` (unknown) - Determines the display of the column header.
   * * `cell` ((item) => unknown) - Determines the display of a cell's content. You receive the current table row
   *   item as an argument.
   ...
   *   * `editConfig.editingCell` ((item, cellContext) => unknown) - Determines the display of a cell's content when inline editing is active on a cell;
   ...
   */
  renderLoaderPending?: (detail: TableProps.RenderLoaderDetail<T>) => unknown;
  /**
   * Renders loader row content for loading state.
   */
  renderLoaderLoading?: (detail: TableProps.RenderLoaderDetail<T>) => unknown;
  /**
   * Renders loader row content for error state.
   */
  renderLoaderError?: (detail: TableProps.RenderLoaderDetail<T>) => unknown;
  /**
   ...
   */
  renderLoaderEmpty?: (detail: TableProps.RenderLoaderEmptyDetail<T>) => unknown;
  /**
   * Renders loader counter that is appended to the loader content in all loader states.
   */
  renderLoaderCounter?: (detail: TableProps.RenderLoaderCounterDetail<T>) => unknown;
}
export declare namespace TableProps {
  ...
  interface EditConfig<T, V = any> {
     ...
     */
    editingCell(item: T, ctx: TableProps.CellContext<any>): unknown;
    /**
  ...
  type ColumnDefinition<T> = {
    ...
    counter?(props: ItemCounterData<T>): unknown;
  ...
  type OnExpandableItemToggle<T> = (event: CustomEvent<ExpandableItemToggleDetail<T>>) => void;
  interface ExpandableItemToggleDetail<T> {
    item: T;
    expanded: boolean;
  }
  type OnGroupSelectionChange<T> = (event: CustomEvent<GroupSelectionChangeDetail<T>>) => void;
